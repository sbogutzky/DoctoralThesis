...
\begin{itemize}
	\item mittlere \ac{HR} 
	\item \acs{RMSSD} der zeitbezogenen statistischen \acs{HRV}-Analyse 
...
Die Ableitung nach Einthoven dient zur Darstellung von Potenzialänderungen in der Frontalebene und ermöglicht u. a. die Identifikation von Herzschlägen, die wir zur Analyse der \acs{HRV} benötigen.

Wie in Abschnitt~\ref{ssub:kardiovaskulare_messungen} zusammengefasst, handelt es sich bei der \acs{RMSSD} um ein physiologisches Merkmal der zeitbezogenen statistischen \acs{HRV}-Analyse, mit dem \citet{Keller2011} bei einer sitzenden Tätigkeit Zusammenhänge mit Flow"=Erleben feststellten. Auf Zusammenhangsanalysen mit frequenzbezogenen \acs{HRV}-Merkmalen verzichtete ich aufgrund der in Abschnitt~\ref{sub:zuordnung} beschriebenen Probleme.
...
\paragraph{Kubios \acs{HRV} zur \acs{HRV}-Analyse:} 

% (fold)
\label{par:kubios_hrv_zur_hrv_analyse}

Nach der Segmentierung der \ac{EKG}-Daten führe ich die Analyse der \acs{HRV} in Kubios \acs{HRV} durch. Kubios \acs{HRV} ist eine benutzerfreundliche Software für die Analyse der \acs{HRV} \citep{Tarvainen2014}. Kubios \acs{HRV} ist eine freie Software und wurde mit MATLAB entwickelt. Mit Kubios \acs{HRV} speichere ich ggf. zeitbezogene, frequenzbezogene und nicht lineare \acs{HRV}-Parameter sowie die Zeitreihe der R"=Spitzen in Textdateien im txt"=Format.
...
Nach der Glättung trennt das R"=Programm die Beschleunigungen in X-, Y-, Z"=Richtung anhand der \ac{MS}-Erkennungen und berechnet den Bewegungsaufwand für jeden einzelnen Bewegungsablauf mit nachfolgender Formel: 
\begin{equation}
...
Um Zusammenhänge zwischen den expliziten Flow"=Merkmalen der \ac{FKS} und impliziten Merkmalen zu untersuchen (Anschnitt~\ref{sec:herangehensweise}, Schritt~2), berechnete ich akkumulierte implizite Merkmale der Kurzzeit-\acs{HRV} (mittlere \ac{HR} und \acs{RMSSD}), des Bewegungsaufwands (mittlerer Bewegungsaufwand und mittlere Doppelschrittfrequenz) und der kardio"=lokomotorische Phasensynchronisation (mittlerer normalisierter Shannon Entropie Index). Als Datengrundlage nutzte ich die fünf Minuten an Daten direkt vor der Befragung mit der \ac{FKS}, da die \ac{FKS} den gegenwärtigen Flow"=Zustand mit Items (Aussagen) wie »Ich fühle mich optimal beansprucht.« abfragt. Dabei halte ich die Länge von fünf Minuten für einen guten Kompromiss, da ich damit die empfohlene Länge für die Berechnungen der Kurzzeit-\acs{HRV} \citep[][S.~360]{TaskForce1996} einhalte und genügend Anlaufzeit nach dem Start des Laufes und nach jeder Unterbrechung durch die Befragung mit der \ac{FKS} bleibt (Kapitel~\ref{cha:flow_erleben_beim_gehen_und_laufen_messen_anforderungen}). Zusätzlich musste ich bei der Datenlänge von fünf Minuten keine \ac{EKG}-Messung aufgrund von vielen Artefakten aus der Datenbasis entfernen, was bei längeren Zeitspannen der Fall gewesen wäre und was zu einer ungleichen Anzahl von expliziten und impliziten Merkmalen geführt hätte.
...
Im dritten Lauf (17.10.) bewertete der Befragte im Gegensatz zu allen anderen Läufen die expliziten Flow"=Merkmale am niedrigsten. Zudem weist der \acs{HRV}-Parameter \acs{RMSSD} eine hohe Varianz im Gegensatz zu den anderen Läufen auf (Abbildung~\ref{fig:ubersicht_nach_laufen_1}, Reihe~2, Spalte~2). 
